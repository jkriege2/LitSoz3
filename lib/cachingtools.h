...
#ifndef LS3CACHINGTOOLS_H
#define LS3CACHINGTOOLS_H
...
/** \brief tag type to configure LS3DataCache for thread-safety
 *  \ingroup ls3tools_concurrency
 */
struct LS3DataCacheThreadSafe {};
/** \brief tag type to configure LS3DataCache for non thread-safety
 *  \ingroup ls3tools_concurrency
 */
struct LS3DataCacheNotThreadSafe {};
 ...
 */
template <class TData, class TKey, typename ThreadSafe=LS3DataCacheThreadSafe, class TKeyInSignature=TKey>
struct LS3DataCache {
    template <typename FF>
    inline LS3DataCache(FF generateData, int maxEntries=10000, double retainFraction=0.8):
        m_maxEntries(maxEntries), m_retainFraction(retainFraction), m_generateData(std::forward<FF>(generateData))
    {

    }
    LS3DataCache()=delete;
    LS3DataCache(const LS3DataCache&)=delete;
    LS3DataCache& operator=(const LS3DataCache&)=delete;
    LS3DataCache(LS3DataCache&&)=default;
    LS3DataCache& operator=(LS3DataCache&&)=default;
    ...
    inline TData get(const typename std::enable_if<std::is_same<LS3DataCacheThreadSafe, TSS>::value, TKeyInSignature>::type& key) {
    ...
    template <typename TSS=ThreadSafe>
    inline TData get(const typename std::enable_if<std::is_same<LS3DataCacheNotThreadSafe, TSS>::value, TKeyInSignature>::type& key) {
        const TKey cacheKey=key;
    ...
    template <typename TSS=ThreadSafe>
    inline bool contains(const typename std::enable_if<std::is_same<LS3DataCacheThreadSafe, TSS>::value, TKeyInSignature>::type& key) const {
        const TKey cacheKey=key;
    ...
    template <typename TSS=ThreadSafe>
    inline bool contains(const typename std::enable_if<std::is_same<LS3DataCacheNotThreadSafe, TSS>::value, TKeyInSignature>::type& key) const {
        const TKey cacheKey=key;
        return m_cache.find(cacheKey)!=m_cache.end();
    }
...
private:
    ...
    inline typename std::enable_if<std::is_same<LS3DataCacheThreadSafe, TSS>::value, int>::type size_impl() const {
        QReadLocker lockR(&m_mutex);
        return m_cache.size();
    }

    template <typename TSS>
    inline typename std::enable_if<std::is_same<LS3DataCacheNotThreadSafe, TSS>::value, int>::type size_impl() const {
        return m_cache.size();
    }
    ...
    inline void cleanCache_notThreadSafe() {
        ...
        const int deleteItems=ls3_boundedRoundTo<int>(1, (1.0-m_retainFraction)*static_cast<double>(m_cache.size()), m_cache.size());
