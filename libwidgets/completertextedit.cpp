...
CompleterTextEditNumberBar::~CompleterTextEditNumberBar() {
}

void CompleterTextEditNumberBar::readSettings(QSettings& settings) {
...
void CompleterTextEditNumberBar::addMarker(int line, itemType type, QString message) {
    itemData d;
    d.line=line;
    d.message=message;
    d.type=type;
...
void CompleterTextEditNumberBar::setEditor(QTextEdit* edtr) {
    editor=edtr;
...
bool CompleterTextEditNumberBar::event(QEvent *event) {
...
CompleterTextEditWidget::CompleterTextEditWidget(QWidget *parent)
...
CompleterTextEditWidget::~CompleterTextEditWidget()
{
}

void CompleterTextEditWidget::setCompleter(QCompleter *completer) {
     if (c)
 ...
 QCompleter *CompleterTextEditWidget::completer() const
 {
     return c;
 }

 void CompleterTextEditWidget::insertCompletion(const QString& completion1)
 {
     if (c->widget() != this)
         return;
 ...
 QString CompleterTextEditWidget::textUnderCursor() const
 {
     QTextCursor tc = textCursor();
 ...
 void CompleterTextEditWidget::focusInEvent(QFocusEvent *e)
 {
     if (c)
         c->setWidget(this);
 ...
 void CompleterTextEditWidget::gotoLine(int line)
 {
...
void CompleterTextEditWidget::cursorChanged() {
...
void CompleterTextEditWidget::processTab(QKeyEvent *e) {
    QTextCursor c(textCursor());
 ...
 void CompleterTextEditWidget::keyPressEvent(QKeyEvent *e) {
...
void CompleterTextEditWidget::indentInc(){
    QTextCursor c(textCursor());
    if (c.selectionStart() == c.selectionEnd())
...
void CompleterTextEditWidget::indentDec(){
    QTextCursor c(textCursor());
    if (c.selectionStart() == c.selectionEnd())
        c.select(QTextCursor::LineUnderCursor);
    if (c.selectionStart() != c.selectionEnd()) {
...
QString CompleterTextEditWidget::removeComments(QString t) {
    //int pos1=0;
    int i=0;
...
bool CompleterTextEditWidget::findFirst(QString phrase, bool searchFromStart, bool matchCase, bool wholeWords){
...
bool CompleterTextEditWidget::findNext(){
...
bool CompleterTextEditWidget::replaceFirst(QString phrase, QString replaceBy, bool searchFromStart, bool matchCase, bool wholeWords, bool replaceAll, bool askBeforeReplace){
    this->searchFromStart=searchFromStart;
    this->matchCase=matchCase;
    this->wholeWords=wholeWords;
    this->searchPhrase=phrase;
...
bool CompleterTextEditWidget::replaceNext(){
...
CompleterTextEdit::CompleterTextEdit(QWidget* parent):
    QWidget(parent)
{
    editor = new CompleterTextEditWidget(this);
    editor->setFrameStyle(QFrame::NoFrame);

    numbers = new CompleterTextEditNumberBar(editor, this);
...
void CompleterTextEdit::readSettings(QSettings& settings) {
