...
void QtLogFile::open_logfile(QString filename, bool writeStore) {
  QMutexLocker locker(mutex);
...
QString QtLogFile::indentify(QString text){
  QMutexLocker locker(mutex);
  QString res="";
...
void QtLogFile::raw_log_text(QString message){
  QMutexLocker locker(mutex);
...
void QtLogFile::log_header(QString caption){
  QMutexLocker locker(mutex);
  int l=caption.size();
  int lall=78-2-l-1;
  do_indent();
...
void QtLogFile::log_dheader(QString caption){
  QMutexLocker locker(mutex);
  int l=caption.size();
  int lall=78-2-l-1;
  do_indent();
...
void QtLogFile::log_text(QString message) {
  QMutexLocker locker(mutex);
  do_indent();
...
void QtLogFile::log_error(QString header, QString message) {
    QMutexLocker locker(mutex);
    log_error_header(header);
    log_error(message+QString("\n"));
    log_error_line();
}
void QtLogFile::log_warning(QString header, QString message) {
    QMutexLocker locker(mutex);
    log_warning_header(header);
    log_warning(message+QString("\n"));
    log_warning_line();
}
void QtLogFile::log_text(QString header, QString message) {
    QMutexLocker locker(mutex);
    log_header(header);
...
void QtLogFile::log_error_linebreak(QString message) {
    log_error(message+QString("\n"));
}
void QtLogFile::log_warning_linebreak(QString message) {
    log_warning(message+QString("\n"));
}
void QtLogFile::log_text_linebreak(QString message) {
    log_text(message+QString("\n"));
}

void QtLogFile::log_error_linebreak_queue(QString message) {
    emit emit_log_error_linebreak(message+QString("\n"));
}

void QtLogFile::log_warning_linebreak_queue(QString message) {
    emit emit_log_warning_linebreak(message+QString("\n"));
}

void QtLogFile::log_text_linebreak_queue(QString message) {
...
void QtLogFile::log_warning(QString message){
    QMutexLocker locker(mutex);
...
void QtLogFile::log_error(QString message){
    QMutexLocker locker(mutex);
...
void QtLogFile::log_error_header(QString caption){
    QMutexLocker locker(mutex);
  currentColor=errorColor;
  currentBold=true;
  log_header(caption);
...
void QtLogFile::log_error_dheader(QString caption){
    QMutexLocker locker(mutex);
  currentColor=errorColor;
  currentBold=true;
  log_dheader(caption);
...
void QtLogFile::log_warning_header(QString caption){
    QMutexLocker locker(mutex);
  currentColor=warningColor;
  log_header(caption);
  currentColor=textColor;
...
void QtLogFile::log_warning_dheader(QString caption){
    QMutexLocker locker(mutex);
  currentColor=warningColor;
  log_dheader(caption);
  currentColor=textColor;
